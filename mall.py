...
class Mall:
	def __init__(self):
		...
		self.shops.extend([shop1_1, shop1_2, shop2_1, shop2_2])
	...
	def getShopListByFloor(self, floor):
		return [shop.as_dict() for shop in self.shops if shop.floor == floor]
		
	def get_workers_by_mall_id(self, mall_id):
		return [worker.as_dict() for worker in self.workers if worker.mall_id == mall_id]
